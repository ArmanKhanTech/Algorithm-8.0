...
import { StarsCanvas, Alert } from "../components";
...
const Edit = () => {
  ...
  const githubLinkedinRegex =
    /^\s*(https?:\/\/)?(www\.)?github\.com\/[a-zA-Z0-9_-]+\s*$/;
  const linkedinRegex =
    /^\s*(https?:\/\/)?(www\.)?linkedin\.com\/in\/[a-zA-Z0-9_-]+\s*$/;
  ...
  if (user && !dataFetched) {
    const checkUserRegistered = async () => {
      var q1 = query(
        collection(db, "teams"),
        where("emailLead", "==", user.email),
      );
      var q2 = query(
        collection(db, "teams"),
        where("emailMember2", "==", user.email),
      );
      var q3 = query(
        collection(db, "teams"),
        where("emailMember3", "==", user.email),
      );
  ...
  const uploadResumeLead = async () => {
    ...
    if (resumeLead && resumeLinkLead == "") {
      try {
        ...
        const resumeRef = ref(
          storage,
          `resumes/${Date.now() + "-" + resumeLead.name}`,
        );
  ...
  const uploadImageLead = async (resumeLeadUrl) => {
    if (imageLead && imageLinkLead == "") {
      try {
        ...
        const imageRef = ref(
          storage,
          `images/${Date.now() + "-" + imageLead.name}`,
        );
  ...
  const uploadResumeMember2 = async (resumeLeadUrl, imageLeadUrl) => {
    if (resumeMember2 && resumeLinkMember2 == "") {
      try {
        ...
        const resumeRef = ref(
          storage,
          `resumes/${Date.now() + "-" + resumeMember2.name}`,
        );
  ...
  const uploadImageMember2 = async (
    resumeLeadUrl,
    imageLeadUrl,
    resumeMember2Url,
  ) => {
    if (imageMember2 && imageLinkMember2 == "") {
      try {
        ...
        const imageRef = ref(
          storage,
          `images/${Date.now() + "-" + imageMember2.name}`,
        );
  ...
  const uploadResumeMember3 = async (
    resumeLeadUrl,
    imageLeadUrl,
    resumeMember2Url,
    imageMember2Url,
    ...
    if (resumeMember3 && resumeLinkMember3 == "") {
      try {
        ...
        const resumeRef = ref(
          storage,
          `resumes/${Date.now() + "-" + resumeMember3.name}`,
        );
  ...
  const uploadImageMember3 = async (
    resumeLeadUrl,
    imageLeadUrl,
    resumeMember2Url,
    imageMember2Url,
    ...
    if (imageMember3 && imageLinkMember3 == "") {
      try {
        ...
        const imageRef = ref(
          storage,
          `images/${Date.now() + "-" + imageMember3.name}`,
        );
    ...
    try {
      if (memberCount == 3) {
        await updateDoc(docRef.ref, {
          ...
          resumeMember2:
            resumeMember2Url != "" ? resumeMember2Url : resumeLinkMember2,
          imageMember2:
            imageMember2Url != "" ? imageMember2Url : imageLinkMember2,
          ...
          resumeMember3:
            resumeMember3Url != "" ? resumeMember3Url : resumeLinkMember3,
          imageMember3:
            imageMember3Url != "" ? imageMember3Url : imageLinkMember3,
  ...
  const onSubmit = async (e) => {
    ...
    if (
      (!contactRegex.test(contactMember2) || !contactMember2) &&
      memberCount > 1
    ) {
    ...
    if (
      (!githubLinkedinRegex.test(githubMember2) || !githubMember2) &&
      memberCount > 1
    ) {
    ...
    if (
      (!linkedinRegex.test(linkedinMember2) || !linkedinMember2) &&
      memberCount > 1
    ) {
    ...
    if (
      (!collegeRegex.test(collegeMember2) || !collegeMember2) &&
      memberCount > 1
    ) {
    ...
    if (resumeLinkMember2 == "" && memberCount > 1) {
      ...
      if (resumeMember2.size > 100 * 1024) {
        setLoading(false);
        setAlertMessage(
          "Resume file size exceeds the limit (100KB) (Member 2).",
        );
        setAlertType("error");
        setShowAlert(true);
        setTimeout(() => {
    ...
    if (imageLinkMember2 == "" && memberCount > 1) {
      ...
      if (imageMember2.size > 100 * 1024) {
        setLoading(false);
        setAlertMessage(
          "Image file size exceeds the limit (100KB) (Member 2).",
        );
        setAlertType("error");
        setShowAlert(true);
        setTimeout(() => {
    ...
    if (
      (!contactRegex.test(contactMember3) || !contactMember3) &&
      memberCount > 2
    ) {
    ...
    if (
      (!githubLinkedinRegex.test(githubMember3) || !githubMember3) &&
      memberCount > 2
    ) {
    ...
    if (
      (!linkedinRegex.test(linkedinMember3) || !linkedinMember3) &&
      memberCount > 2
    ) {
    ...
    if (
      (!collegeRegex.test(collegeMember2) || !collegeMember3) &&
      memberCount > 2
    ) {
    ...
    if (resumeLinkMember3 == "" && memberCount > 2) {
      ...
      if (resumeMember3.size > 100 * 1024) {
        setLoading(false);
        setAlertMessage(
          "Resume file size exceeds the limit (100KB) (Member 3).",
        );
        setAlertType("error");
        setShowAlert(true);
        setTimeout(() => {
    ...
    if (imageLinkMember3 == "" && memberCount > 2) {
      ...
      if (imageMember3.size > 100 * 1024) {
        setLoading(false);
        setAlertMessage(
          "Image file size exceeds the limit (100KB) (Member 3).",
        );
        setAlertType("error");
        setShowAlert(true);
        setTimeout(() => {
        ...
        {isRegistered && isLead ? (
            ...
            <div className="flex justify-center mt-2">
              {!member2Exists && !member3Exists ? (
                <p className="text-xl font-semibold text-green-600">
                  Only you have registered so far.
                </p>
              ) : !member3Exists ? (
                <p className="text-xl font-semibold text-green-600">
                  Only you and one more member have registered so far.
                </p>
              ) : (
                <p className="text-xl font-semibold text-green-600">
          ...
          <div className="flex bg-blur border-orange-600 border m-3 rounded-xl p-3 justify-center">
            <h1 className="text-2xl font-bold text-white">
              Only lead can edit the details.
            </h1>
          </div>
        ) : (
          <div className="flex bg-blur border-orange-600 border m-3 rounded-xl p-3 justify-center">
            <h1 className="text-2xl font-bold text-white">
              You are not registered in any team.
            </h1>
          </div>
        )}
          ...
          <form className="grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <aside className="border border-orange-600 rounded-2xl p-3 mt-[26px] mx-3 mb-3 flex-1">
              <div className="bg-blur rounded-2xl h-full p-4">
                ...
                <ul className="list-disc mt-4 list-inside text-xl">
                  ...
                  <li>
                    New members cannot be added here only details of existing
            ...
            {memberCount > 0 ? (
              <div className="flex justify-center text-center md:text-left font-bold">
                <button
                  className={`mt-3 mx-2 lg:mx-3 text-lg bg-orange-600 hover:bg-orange-800 px-4 py-2 text-white font-bold uppercase rounded-xl`}
                  type="submit"
                  style={{
                    width: "100%",
                    display: "flex",
            ...
            {memberCount > 0 ? (
              <div className="flex justify-center text-center md:text-left font-bold">
                <Link
                  to="/dashboard"
                  className={`mt-3 mx-2 lg:mx-3 text-lg bg-white px-4 py-2 text-orange-600 uppercase rounded-xl`}
                  style={{
                    width: "100%",
                    display: "flex",
                    justifyContent: "center",
